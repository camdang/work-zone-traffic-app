...
def compute_delay(
    ...
    time_points = [t for t in range(num_hours + 1)]
    ...
    if demand <= wz_capacity:
        speed_reduction_delay = max(
            0.0, (wz_length / wz_speed - wz_length / posted_speed) * 60
        )
        queue_over_time = [0.0] * len(time_points)
        total_arrivals = demand * analysis_duration
        total_delay_veh_hours = (speed_reduction_delay / 60.0) * total_arrivals
    ...
    total_arrivals = demand * analysis_duration
    avg_delay_per_vehicle = (total_delay_veh_hours / total_arrivals) * 60.0 if total_arrivals > 0 else 0.0
